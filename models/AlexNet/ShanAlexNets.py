...
class AlexNetLinearShan(pl.LightningModule):

        def __init__(self, num_classes: int = 10, in_channels: int = 3,  dropout: float = 0.5) -> None:
            super(AlexNetLinearShan, self).__init__()
...
class AlexNetConvShan(pl.LightningModule):

        def __init__(self, num_classes: int = 10, in_channels: int = 3,  dropout: float = 0.5) -> None:
            super(AlexNetConvShan, self).__init__()
            self.features = nn.Sequential(
...
class AlexNetShan(pl.LightningModule):

        def __init__(self, num_classes: int = 10, in_channels: int = 3,  dropout: float = 0.5) -> None:
            super(AlexNetShan, self).__init__()
            self.features = nn.Sequential(
            Conv2dShannon(in_channels, 64, kernel_size=11, stride=4, padding=2),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2),
