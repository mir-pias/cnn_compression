...
class _DenseLayer(nn.Module):
    def __init__(
        ...
        self.conv1 = Conv2dDFT(num_input_features, bn_size * growth_rate, kernel_size=1, stride=1, bias=False)

        self.norm2 = ComplexBatchNorm2d(bn_size * growth_rate)
        self.cardioid2 = Cardioid()
        self.conv2 = Conv2dDFT(bn_size * growth_rate, growth_rate, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def bn_function(self, inputs: List[Tensor]) -> Tensor:
        ...
        bottleneck_output = self.conv1(self.cardioid1(self.norm1(concated_features)))  # noqa: T484
    ...
    def forward(self, input: Tensor) -> Tensor:  # noqa: F811
        ...
        new_features = self.conv2(self.cardioid2(self.norm2(bottleneck_output)))
        if self.drop_rate > 0:
...
class DenseNetConvDFT(pl.LightningModule):
    ...
    def __init__(
        ...
        self.features = nn.Sequential(
            OrderedDict(
                [
                    ("conv0", Conv2dDFT(in_channels, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
...
def _densenetConvDFT(
    ...
    **kwargs: Any,
    ...
    model = DenseNetConvDFT(growth_rate, block_config, num_init_features, num_classes, **kwargs)
    return model

def densenet121ConvDFT(*,num_classes , **kwargs: Any) -> DenseNetConvDFT:

    return _densenetConvDFT(32, (6, 12, 24, 16), 64, num_classes, **kwargs)

def densenet201ConvDFT(*,num_classes , **kwargs: Any) -> DenseNetConvDFT:

    return _densenetConvDFT(32, (6, 12, 48, 32), 64, num_classes, **kwargs)