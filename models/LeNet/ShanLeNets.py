...
class LeNetLinearShan(pl.LightningModule):
        def __init__(self, num_classes:int =10, in_channels: int = 1):
            super(LeNetLinearShan, self).__init__()
...
class LeNetConvShan(pl.LightningModule):
        def __init__(self, num_classes:int =10, in_channels: int = 1):
            super(LeNetConvShan, self).__init__()
            self.features = nn.Sequential(
...
class LeNetShan(pl.LightningModule):
        def __init__(self, num_classes:int =10, in_channels: int = 1):
            super(LeNetShan, self).__init__()
            self.features = nn.Sequential(
                Conv2dShannon(in_channels, 6, 5),
                nn.ReLU(),
                nn.MaxPool2d(kernel_size=2),
