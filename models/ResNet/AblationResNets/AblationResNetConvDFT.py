...
class AblationResNetConvDFT(pl.LightningModule):
    def __init__(
        self,
...
def _ablationResnetConvDFT(
    block: Type[Union[BasicBlock, Bottleneck]],
    layers: List[int],
    num_classes: int,
    **kwargs: Any,
) -> AblationResNetConvDFT:

    model = AblationResNetConvDFT(block, layers, num_classes, **kwargs)

    return model

def ablationResnet18ConvDFT(*, num_classes, **kwargs: Any) -> AblationResNetConvDFT:

    return _ablationResnetConvDFT(BasicBlock, [2, 2, 2, 2], num_classes, **kwargs)

def ablationResnet50ConvDFT(*, num_classes, **kwargs: Any) -> AblationResNetConvDFT:

    return _ablationResnetConvDFT(Bottleneck, [3, 4, 6, 3], num_classes, **kwargs)